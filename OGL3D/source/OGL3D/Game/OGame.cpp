...
void OGame::onUpdateInternal()
{
	...
	// jeg har problem med at når kuben ikke er i posisjon (0,0,0) so roterer kuben i en rar vei men jeg ikke hvordan og fiske det
	temp.setIdentity();
	...
	temp.setIdentity();
	temp.setRotationZ(m_rotZ);
	world *= temp;

	temp.setIdentity();
	temp.setTranslation(OVec3(m_posX, m_posY, m_posZ));
	world *= temp;
